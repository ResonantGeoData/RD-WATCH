...
# TODO: Parameterize
rgd_endpoint = "http://localhost:8080"
...
def main():
    ...
    upload_to_rgd_scoring(**vars(parser.parse_args()))
...
def upload_to_rgd_scoring(
    ...
    # Check that our run doesn't already exist
    model_run_results_url = f"{rgd_endpoint}/api/scoring/model-runs/"
    cookies = None
    if rgd_auth_cookie:
        cookies = {"AWSELBAuthSessionCookie-0": rgd_auth_cookie}
    model_runs_result = requests.get(
    ...
    if existing_model_run is not None:
        ...
        post_model_url = f"{rgd_endpoint}/api/scoring/model-runs/"
    ...
    post_site_url = f"{rgd_endpoint}/api/scoring/model-runs/{model_run_id}/site-model/"
    print(site_models_glob)
    for site_file in glob(site_models_glob):
        print("Uploading '{}' ..".format(site_file))
        try:
