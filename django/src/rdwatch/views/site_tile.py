...
class SiteTileSchema(AutoSchema):
...
class RetrieveSiteTile(APIView):

    permission_classes = [permissions.AllowAny]
    schema = SiteTileSchema()
    action = "retrieve"
    ...
    def get(
        ...
        *args,
        ...
        if pk is None or z is None or x is None or y is None:
            raise TypeError()
        if not Site.objects.filter(pk=pk).exists():
            raise NotFound()
        agg = Site.objects.filter(pk=pk).aggregate(tile=VectorTile("geometry", z, x, y))
        data = agg["tile"]
